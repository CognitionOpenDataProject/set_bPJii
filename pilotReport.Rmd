...
## Step 1.5. Replicating with sent Matlab codes 
...
```{r}
compareValues(reportedValue = 9.7, obtainedValue = 9.7)
...
The script was organized such that several lines of code had to be re-run with different conditions specified (described in comments). I added a few sections to the code to be sure I ran it in the correct order, as objects were named the same and could easily be erroneously assigned to the wrong condition if run out of order. 
...
Subset fixation by ROI, then melt:
```{r}
...
This does not match what was reported, but likely due to insufficient information about what points they used. 
...
## Appendix: R preprocessing Attempts
### Pre-processing
